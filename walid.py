...
def read_control():
    ...
    ctrl = []
    global A1, A2, B4, I1, I2, I3, I4, P1, P2, P3, P4, Q1
    A1 = A2 = np.nan        # raster resolution
    I3 = 1.8                # interpolation window length (I3 x dC)
...
def redist_vertices(thalweg, target_distance):
    '''Redistribute points along line'''
    thalweg_array = np.array(thalweg)  # Convert input to a numpy array if it's not already
    redistributed_points = []

    for i in range(len(thalweg_array) - 1):
        p1 = thalweg_array[i]
        p2 = thalweg_array[i + 1]
        segment_vector = p2 - p1
        segment_length = np.linalg.norm(segment_vector)

        redistributed_points.append(p1)

        if segment_length > target_distance:
            num_interpolated_points = int(segment_length // target_distance)
            for j in range(1, num_interpolated_points + 1):
                t = j * target_distance / segment_length
                interpolated_point = p1 + t * segment_vector
                redistributed_points.append(interpolated_point)

    redistributed_points.append(thalweg_array[-1])
    return np.array(redistributed_points)


def convec_hull(points, alpha, only_outer=True):
    '''convex hull'''
...
def dist2d(xi, yi, xyz):
    ...
    D = np.sqrt((xi - xyz[:, 0])**2 + (yi - xyz[:, 1])**2)
    return D


def thalweg_ang(tw):
    """Calculate thalweg angles and add line equation parameters."""
    # Initialize columns for angle, slope, and line equation parameters
    tw = np.hstack((tw, np.zeros((len(tw), 5))))

    # Calculate dx and dy using vectorized operations
    dx = np.zeros(len(tw))
    dy = np.zeros(len(tw))
    ...
    dx[-1] = tw[-1, 0] - tw[-2, 0]
    dy[-1] = tw[-1, 1] - tw[-2, 1]
    dx[1:-1] = 0.5 * (tw[2:, 0] - tw[:-2, 0])
    dy[1:-1] = 0.5 * (tw[2:, 1] - tw[:-2, 1])

    # Calculate slope and angle (tw[:, 2] and tw[:, 3])
    with np.errstate(divide='ignore', invalid='ignore'):
        tw[:, 2] = dy / dx  # Slope
        tw[:, 3] = -np.arctan(1 / tw[:, 2])  # Angle

    # Calculate line equation parameters A, B, and C
    tw[0, 4] = 0.0
    tw[1:, 4] = -np.diff(tw[:, 1]) / np.diff(tw[:, 0])  # A
    tw[:, 5] = 1.0  # B
    tw[1:, 6] = -(tw[1:, 2] + tw[1:, 1] * tw[1:, 4])  # C
    tw[0, 6] = 0.0  # Initialize C at the first index
...
tw = cca(tw, 1)                # Chaikin's iterations
...
if Q1 == 0:
    ...
    boundary = convec_hull(xyz[:, 0:2], A2, only_outer=True)
    boundary = cca(boundary[0::1, :], 1)  # smoothing
